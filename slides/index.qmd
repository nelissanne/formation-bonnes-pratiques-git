...
:::{.incremental}
- [**https**]{.orange}
  - `git clone https://git.drees.fr/username/projet.git`
  - authentification username/token à chaque *push*
  - en local, les identifiants peuvent être stockés dans la dénomination de l'origin
  - le token est alors lu dans un .txt **non indexé**

- [**ssh**]{.orange}
*bloqué à la Drees*
  - `git clone git@git.drees.fr:username/projet.git`
...
Pour une proposition plus complexe, voir le [GitLab flow](https://docs.gitlab.com/ee/topics/gitlab_flow.html).
